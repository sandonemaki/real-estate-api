...
@Injectable()
export class RealEstateDataSourceRepository
  implements IRealEstateDataSourceRepository
{
  async fetchRealEstateValue(
    query: EstateQueryDto,
  ): Promise<RealEstateValueData> {
    ...
    if (response.status >= 200 && response.status < 300) {
      ...
      if (response.data.statusCode && response.data.statusCode !== '200') {
        const errorResponse = this.handleErrorResponse(
          parseInt(response.data.statusCode),
        );
        throw new ResasApiException(
          errorResponse.message,
          errorResponse.statusCode,
